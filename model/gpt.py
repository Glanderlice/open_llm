...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 权重共享：输入token_embedding与输出token_embedding, 可节省大量参数
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))  # ignore_index=-1, 默认-100
        else:
            # 推理：只保留最后一个时间步可以优化推理耗时
            logits = self.lm_head(x[:, [-1], :])  # [-1]可以保留时间维度, 如果是-1则不会保留
            loss = None
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits[:, -1, :] / temperature  # logits.shape变成(B,vocal_size)
